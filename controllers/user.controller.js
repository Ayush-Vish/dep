  ...
  try {
    ...
    const user = await userModel.create({
      ...
      avatar: {
        public_id: email,
        secure_url:
          "https://images.pexels.com/lib/avatars/grey.png?w=130&h=130&fit=crop&dpr=1",
      },
    });
    if (!user) {
      return next(
    ...
    if (req.file) {
      try {
        ...
        console.log(error);
        return next(
          new Apperror(error || "File not uploaded Successfully"),
          500
        );
      }
    } else {
...
const updateUser = async (req, res, next) => {
  try {
    ...
    if (usernameExists) {
      return next(
        new Apperror("Username Already Exists Try Something new", 400)
      );
    }
